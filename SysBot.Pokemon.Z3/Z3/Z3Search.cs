...
namespace SysBot.Pokemon
{
    ...
    {
        public static SeedSearchResult GetFirstSeed(uint ec, uint pid, int[] ivs, string name, string ot, int gender, int abilityNo, int ability, int nature)
        {
            ...
            foreach (var seed in seeds)
            {
                ...
                for (int i = 1; i <= 5; i++) // fixed IV count
                {
                    if (IsMatch(seed, ivs, i))
                        return new SeedSearchResult(Z3SearchResult.Success, seed, i, ivs, name, ot, gender, abilityNo, ability, nature);
                }
                hasClosest = true;
                closest = seed;
            }
            ...
            if (hasClosest)
                return new SeedSearchResult(Z3SearchResult.SeedMismatch, closest, 0, ivs, name, ot, gender, abilityNo, ability, nature);
            return SeedSearchResult.None;
        }

        public static IList<SeedSearchResult> GetAllSeeds(uint ec, uint pid, int[] ivs, string name, string ot, int gender, int abilityNo, int ability, int nature)
        {
            ...
            foreach (var seed in seeds)
            {
                ...
                for (int i = 1; i <= 5; i++) // fixed IV count
                {
                    if (IsMatch(seed, ivs, i))
                    {
                        result.Add(new SeedSearchResult(Z3SearchResult.Success, seed, i, ivs, name, ot, gender, abilityNo, ability, nature));
                        added = true;
                    }
                }
                if (!added)
                    result.Add(new SeedSearchResult(Z3SearchResult.SeedMismatch, seed, 0, ivs, name, ot, gender, abilityNo, ability, nature));
            }
